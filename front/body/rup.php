<table class="table table-borderless">
	...
	<tr class="hr_section topic_section">
		<td class="text-center" colspan="4">
			<hr>
			<h6>Семестры, в которых осуществяется освоение дисциплины</h6>
			<hr>
		</td>
	</tr>
			...
			<table>
				<tr>
					<td style="vertical-align: middle">
						<span>по очной форме обучения</span>
					</td>
					<td>
						<?php for ($i = 1; $i <= 8; $i++) {
							echo '<label class="intramural_semester-label mx-1 mb-0" for="intramural_semester' . $i . '">' . $i . ' <input type="checkbox"  data-value="' . $i . '" class="intramural_semester-checkbox" id="intramural_semester' . $i . '"></label>';
						} ?>
						<button class="btn btn-primary intramural_semester-button">Сброс</button>
					</td>
	...
	<tr>
		<td colspan="4">
			<table>
				<tr>
					<td style="vertical-align: middle">
						<span>по заочной форме обучения</span>
					</td>
					<td>
						<?php for ($i = 1; $i <= 8; $i++) {
							echo '<label class="absentia_semester-label mx-1 mb-0"  for="absentia_semester' . $i . '">' . $i . ' <input class="absentia_semester-checkbox" data-value="' . $i . '" type="checkbox" id="absentia_semester' . $i . '"></label>';
						} ?>
						<button class="btn btn-primary absentia_semester-button">Сброс</button>
					</td>
				</tr>
			</table>
		</td>
	...
	<tr class="hr_section topic_section">
		<td class="text-center" colspan="4">
			<hr>
			<h6>Содержание дисциплины, структурированное по темам (разделам)</h6>
			<hr>
		</td>
	</tr>
...
<script>
	$(document).ready(function() {
		function resetCheck(inputs) {
			inputs.each(function(i, item) {
				$(item).prop('checked', false);
			})
		}

		$('.intramural_semester-button').click(function(e) {
			e.preventDefault();
			resetCheck($('.intramural_semester-input'))
		});

		$('.absentia_semester-button').click(function(e) {
			e.preventDefault();
			resetCheck($('.absentia_semester-input'))
		});
			...
			if ((pulpit_value === null) && (pulpit_value === '')) {
				...
				$.post(
					"../back/switch_functions.php", {
						functionname: 'get_discipline_list',
						param: pulpit_value
					},
					function(info) {
						$('#input_discipline').html(info);
					}
				);
			}
			$.post(
				"../back/switch_functions.php", {
					functionname: 'get_institute',
					param: pulpit_value
				},
				function(info) {
					$('#info_institute').text(info);
				}
			);
		});
		...
		$("#input_discipline").change(function() {
			...
			$.post(
				"../back/switch_functions.php", {
					functionname: 'get_part',
					param: discipline_value
				},
				function(info) {
					$('#info_part').text(info + ' части');
				}
			);
		});
		...
		$("#input_course").change(function() {
			...
			if ((course_value === null) && (course_value === '')) {
				...
				$.post(
					"../back/switch_functions.php", {
						functionname: 'get_profile_list',
						param: course_value
					},
					function(info) {
						$('#input_profile').html(info);
					}
				);
				$.post(
					"../back/switch_functions.php", {
						functionname: 'get_fgos_id',
						param: course_value
					},
					function(info) {
						if ((info !== null) && (info !== '')) {
		...
		$("#alert_form_save_rup").click(function() {
			...
			var course_value = $("#input_course").val();
			if ((course_value === null) || (course_value === '')) {
				errors += ("Направление не указано. ");
			};

			var profile_value = $("#input_profile").val();
			if ((profile_value === null) || (profile_value === '')) {
				errors += ("Профиль не указан. ");
			};

			var fgos_value = $("#info_fgos_id").text();
			if ((fgos_value === null) || (fgos_value === '')) {
				errors += ("ФГОС не указан. ");
			};
			...
			var pulpit_value = $("#input_pulpit").val();
			if ((pulpit_value === null) || (pulpit_value === '')) {
				errors += ("Кафедра не указана. ");
			};

			var discipline_value = $("#input_discipline").val();
			if ((discipline_value === null) || (discipline_value === '')) {
				errors += ("Дисциплина не указана. ");
			};

			var goal_value = $("#input_goal").val();
			if ((goal_value === null) || (goal_value === '')) {
				errors += ("Цель не указана. ");
			};
