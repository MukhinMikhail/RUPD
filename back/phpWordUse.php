...
function fill_parameters()
{
	...
	$document->setValue('seminars_form1', $_POST["seminars_form1"]);
	$document->setValue('seminars_form2', $_POST["seminars_form2"]);
	...
	$result = mysqli_query($link, "	select 	time 
										from 	disciplines 
										where 	discipline_id=" . $_POST["discipline"] . "");
	while ($row = mysqli_fetch_array($result)) {
		$document->SetValue('main_time', $row[0]);
		/* Если ye_time Нужно будет округлять вставить вместо регулярного выражения */
		/* round((int) $row[0] / 36), 1) */
	$document->SetValue('ye_time', preg_replace('/(\..{1}).*/', '$1', (int) $row[0] / 36));
	};
